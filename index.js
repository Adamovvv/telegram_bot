...
bot.onText(/\/start/, (msg) => {
  ...
  const keyboard = {
    reply_markup: {
      ...
      keyboard: [
        ['🚚 Крупный заказ', '📦 Мои заказы'],
        ['❓ Помощь', '📢 Подписка'],
      ],
    },
  };

  bot.sendMessage(chatId, 'Добро пожаловать в Brooklyn Tuning. Чтобы посмотреть товары, нажмите на кнопку Каталог.', keyboard);
});
...
// Инициализация массива подписанных пользователей
...
bot.onText(/Подписка/, (msg) => {
  ...
  if (!subscribedUsers.includes(chatId)) {
    // Добавляем пользователя в массив подписанных
    subscribedUsers.push(chatId);

    // Отправляем сообщение с запросом подтверждения подписки и кнопкой "Подписаться"
    bot.sendMessage(
      chatId,
      'Вы действительно хотите подписаться на уведомления?',
      {
        reply_markup: {
          inline_keyboard: [
            [
    ...
    // Если пользователь уже подписан, отправляем сообщение об этом
    bot.sendMessage(chatId, 'Вы уже подписаны на уведомления.');
  }
});

// Обработчик для события callback_query
bot.on('callback_query', (callbackQuery) => {
  const chatId = callbackQuery.message.chat.id;
  const messageId = callbackQuery.message.message_id;
  const data = callbackQuery.data;

  // Проверяем, была ли нажата кнопка "Подписаться"
  if (data === 'confirm_subscription') {
    // Отправляем сообщение подтверждения
    bot.sendMessage(chatId, 'Теперь вы подписаны на уведомления. Спасибо за подписку.');

    // Удаляем кнопку "Подписаться" из сообщения
...
// Обработчик для команды "Рассылка"
bot.onText(/^(Рассылка)$/, (msg) => {
  const chatId = msg.chat.id;

  // Проверяем, является ли пользователь администратором
  if (chatId.toString() !== adminChatId) {
    bot.sendMessage(chatId, 'У вас нет доступа к рассылке.');
    return;
  }

  // Отправляем сообщение для ввода текста рассылки
  bot.sendMessage(chatId, 'Введите текст для рассылки:');

  // Ждем ответа от пользователя
  bot.once('message', (response) => {
    // Обрабатываем текст рассылки
    const broadcastText = response.text;

    // Отправляем уведомление всем подписанным пользователям
    sendNotificationToSubscribedUsers(broadcastText);
...
// Функция для отправки уведомления всем подписанным пользователям
function sendNotificationToSubscribedUsers(notificationText) {
  try {
    // Итерируемся по каждому подписанному пользователю и отправляем уведомление
    for (const userId of subscribedUsers) {
      bot.sendMessage(userId, notificationText);
    }

    console.log('Уведомление успешно отправлено всем подписанным пользователям.');
  } catch (error) {
